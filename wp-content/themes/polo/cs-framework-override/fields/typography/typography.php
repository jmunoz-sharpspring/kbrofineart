...
class CSFramework_Option_typography extends CSFramework_Options {
	...
	public function output() {
		...
		$is_height    = ( isset( $this->field['height'] ) && $this->field['height'] === false ) ? false : true;
		...
		$font_height   = isset( $value['height'] ) ? $value['height'] : '';
		$font_color    = isset( $value['color'] ) ? $value['color'] : '';
		...
		if ( is_object( $google_json ) ) {
			...
			echo '<input type="number" name="' . $this->element_name( '[size]' ) . '" value="' . esc_attr( $font_size ) . '" placeholder="font-size" />';
			echo '</label>';

			if ( ! empty( $is_height )  ) {
				echo '<label class="cs-typography-height cs-pseudo-field">';
				echo '<input type="text" name="' . $this->element_name( '[height]' ) . '" value="' . esc_attr( $font_height ) . '" placeholder="line-height" />';
				echo '</label>';
			}
			if ( ! empty( $is_color )  ) {
				echo '<label class="cs-typography-color cs-pseudo-field cs-field-color_picker">';
				echo '<input type="text" name="' . $this->element_name( '[color]' ) . '" value="' . esc_attr( $font_color ) . '"' . $this->element_class( 'cs-field-color-picker' ) . ' />';
				echo '</label>';
			}
			echo '</fieldset>';
			...
			echo esc_html__( 'Error! Can not load json file.', 'polo' );
