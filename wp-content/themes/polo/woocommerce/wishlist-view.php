			...
			<div class="table table-condensed table-striped">
				...
				if ( ! empty( $page_title ) ) :
					...
					<div class="hidden-title-form">
						<input type="text" value="<?php polo_render($page_title) ?>" name="wishlist_name" />
						<button>
				...
				<table class="cart wishlist_table table" data-pagination="<?php echo esc_attr( $pagination ) ?>" data-per-page="<?php echo esc_attr( $per_page ) ?>" data-page="<?php echo esc_attr( $current_page ) ?>" data-id="<?php echo ( is_user_logged_in() ) ? esc_attr( $wishlist_meta['ID'] ) : '' ?>" data-token="<?php echo ( ! empty( $wishlist_meta['wishlist_token'] ) && is_user_logged_in() ) ? esc_attr( $wishlist_meta['wishlist_token'] ) : '' ?>">
					...
					if ( count( $wishlist_items ) > 0 ) :
						foreach ( $wishlist_items as $item ) :
							...
							if ( $product !== false && $product->exists() ) :
								...
								?>
								<tr id="yith-wcwl-row-<?php polo_render($item['prod_id']) ?>" data-row-id="<?php polo_render($item['prod_id']) ?>">
									<?php if ( $show_cb ) : ?>
										<td class="product-checkbox">
											<input type="checkbox" value="<?php echo esc_attr( $item['prod_id'] ) ?>" name="add_to_cart[]" <?php polo_render( $product->product_type != 'simple' ? 'disabled="disabled"' : '' ); ?>/>
										</td>
									<?php endif ?>
									...
									<?php if ( $show_price ) : ?>
										<td class="cart-product-price">
											...
											if ( is_a( $product, 'WC_Product_Bundle' ) ) {
												...
												polo_render($product->get_price_html());
											} else {
					...
					if ( ! empty( $page_links ) ) : ?>
						<tr class="pagination-row">
							<td colspan="<?php echo esc_attr( $column_count ) ?>"><?php polo_render($page_links) ?></td>
						</tr>
					<?php endif ?>
					</tbody>
				...
				<?php if ( $wishlist_meta['is_default'] != 1 ): ?>
					<input type="hidden" value="<?php polo_render($wishlist_meta['wishlist_token']) ?>" name="wishlist_id" id="wishlist_id">
				<?php endif; ?>
...
<?php if ( $additional_info ): ?>
	<div id="ask_an_estimate_popup">
		<form action="<?php polo_render($ask_estimate_url) ?>" method="post" class="wishlist-ask-an-estimate-popup">
