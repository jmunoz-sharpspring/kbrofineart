...
class Breadcrumb_Trail {
	...
	protected function set_labels() {
		$defaults = array(
			'browse'              => esc_html__( 'Browse:',                               'polo' ),
			'aria_label'          => esc_attr_x( 'Breadcrumbs', 'breadcrumbs aria label', 'polo' ),
			'home'                => esc_html__( 'Home',                                  'polo' ),
			'error_404'           => esc_html__( '404 Not Found',                         'polo' ),
			'archives'            => esc_html__( 'Archives',                              'polo' ),
			// Translators: %s is the search query.
			'search'              => esc_html__( 'Search results for: %s',                'polo' ),
			// Translators: %s is the page number.
			'paged'               => esc_html__( 'Page %s',                               'polo' ),
			// Translators: %s is the page number.
			'paged_comments'      => esc_html__( 'Comment Page %s',                       'polo' ),
			// Translators: Minute archive title. %s is the minute time format.
			'archive_minute'      => esc_html__( 'Minute %s',                             'polo' ),
			// Translators: Weekly archive title. %s is the week date format.
			'archive_week'        => esc_html__( 'Week %s',                               'polo' ),
	...
	protected function add_minute_hour_archive_items() {
		...
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->labels['archive_minute_hour'], get_the_time( esc_html_x( 'g:i a', 'minute and hour archives time format', 'polo' ) ) );
	}
	/**
	...
	protected function add_minute_archive_items() {
		...
		// Add the minute item.
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->labels['archive_minute'], get_the_time( esc_html_x( 'i', 'minute archives time format', 'polo' ) ) );
	}
	/**
	...
	protected function add_hour_archive_items() {
		...
		// Add the hour item.
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->labels['archive_hour'], get_the_time( esc_html_x( 'g a', 'hour archives time format', 'polo' ) ) );
	}
	/**
	...
	protected function add_day_archive_items() {
		...
		$year  = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'polo' ) ) );
		$month = sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'polo' ) ) );
		$day   = sprintf( $this->labels['archive_day'],   get_the_time( esc_html_x( 'j', 'daily archives date format',   'polo' ) ) );
	...
	protected function add_week_archive_items() {
		...
		$year = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'polo' ) ) );
		$week = sprintf( $this->labels['archive_week'],  get_the_time( esc_html_x( 'W', 'weekly archives date format', 'polo' ) ) );
		// Add the year item.
		$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_year_link( get_the_time( 'Y' ) ) ), $year );
		// Add the week item.
		if ( is_paged() )
	...
	protected function map_rewrite_tags( $post_id, $path ) {
		...
		if ( is_array( $matches ) ) {
			...
			foreach ( $matches as $match ) {
				...
				if ( '%year%' == $tag )
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_year_link( get_the_time( 'Y', $post_id ) ) ), sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'polo' ) ) ) );
				// If using the %monthnum% tag, add a link to the monthly archive.
				elseif ( '%monthnum%' == $tag )
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) ), sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'polo' ) ) ) );
				// If using the %day% tag, add a link to the daily archive.
				elseif ( '%day%' == $tag )
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) ), sprintf( $this->labels['archive_day'], get_the_time( esc_html_x( 'j', 'daily archives date format', 'polo' ) ) ) );
...
class buddypress_Breadcrumb_Trail extends Breadcrumb_Trail
{
	...
	public function do_trail_items()
	{
		...
		if ( !empty( $bp->displayed_user->fullname ) ) { // looking at a user or self
			$this->items[] ='<a href="' . esc_url(bp_get_members_directory_permalink()) . '">' . get_the_title( $bp->pages->members->id ) . '</a>';
			$this->items[] = '<a href="' . esc_url($bp->displayed_user->domain) . '" title="' . strip_tags( $bp->displayed_user->userdata->display_name) . '">' . strip_tags( $bp->displayed_user->userdata->display_name ) .'</a>';

		} else if ( $bp->is_single_item ) { // we're on a single item page
			$this->items[] = '<a href="'. esc_url(get_permalink( $bp->pages->{$bp->current_component}->id )) .'" title="'. esc_attr( strip_tags( get_the_title( $bp->pages->{$bp->current_component}->id ) ) ) .'">'. get_the_title( $bp->pages->{$bp->current_component}->id ) .'</a>';
			$this->items[] = '<a href="'. esc_url(get_permalink() . '" title="' . esc_attr( $bp->bp_options_title )) . '">' . $bp->bp_options_title . '</a>';
			...
			$trail_end = esc_html__( 'Create an Account', 'polo' );
		} else if ( bp_is_activation_page() ) {
			$trail_end = esc_html__( 'Activate your Account', 'polo' );
		} else if ( bp_is_group_create() ) {
			$this->items[] = '<a href="' . esc_url(get_permalink( $bp->pages->groups->id )) . '" title="' . esc_attr( get_the_title( $bp->pages->groups->id ) ) . '">' . get_the_title( $bp->pages->groups->id ) . '</a>';

			$trail_end = esc_html__( 'Create a Group', 'polo' );
			...
			$trail_end = esc_html__( 'Create a Blog', 'polo' );
		}
		if ($trail_end) {
