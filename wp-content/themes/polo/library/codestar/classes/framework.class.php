...
class CSFramework extends CSFramework_Abstract {
  ...
  public function validate_save( $request ) {
    ...
    if ( isset( $request['import'] ) && ! empty( $request['import'] ) ) {
      ...
      $add_errors[] = $this->add_settings_error( esc_html__( 'Success. Imported backup options.', 'polo' ), 'updated' );
    }

    // reset all options
    if ( isset( $request['resetall'] ) ) {
      $add_errors[] = $this->add_settings_error( esc_html__( 'Default options restored.', 'polo' ), 'updated' );
      return;
    }
    ...
    if ( isset( $request['reset'] ) && ! empty( $section_id ) ) {
      ...
      $add_errors[] = $this->add_settings_error( esc_html__( 'Default options restored for only this section.', 'polo' ), 'updated' );
    }
  ...
  public function admin_page() {
    ...
    if ( isset($this->settings['framework_title']) && !empty($this->settings['framework_title']) ) {
      echo '<h1>' . $this->settings['framework_title'] . '</h1>';
    }
      echo '<fieldset>';

      polo_render( $this->settings['ajax_save'] ? '<span id="cs-save-ajax">'. esc_html__( 'Settings saved.', 'polo' ) .'</span>' : '' );

      submit_button( esc_html__( 'Save', 'polo' ), 'primary cs-save', 'save', false, array( 'data-save' => esc_html__( 'Saving...', 'polo' ) ) );
      submit_button( esc_html__( 'Restore', 'polo' ), 'secondary cs-restore cs-reset-confirm', $this->unique .'[reset]', false );

      if( isset($this->settings['show_reset_all']) && $this->settings['show_reset_all'] ) {
        submit_button( esc_html__( 'Reset All Options', 'polo' ), 'secondary cs-restore cs-warning-primary cs-reset-confirm', $this->unique .'[resetall]', false );
      }

      echo '</fieldset>';
      echo ( empty( $has_nav ) ) ? '<a href="#" class="cs-expand-all"><i class="fa fa-eye-slash"></i> '. esc_html__( 'show all options', 'polo' ) .'</a>' : '';
