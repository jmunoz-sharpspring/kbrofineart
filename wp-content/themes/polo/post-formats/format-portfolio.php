...
if(isset($post_meta['portfolio_description']) && !empty($post_meta['portfolio_description'])){
	...
	$folio_excerpt = polo_do_multilang_text($folio_excerpt);
}else{
	$folio_excerpt = '';
}
...
polo_portfolio_force_thumb_size( $width, $height );
...
<div class="portfolio-item <?php echo esc_attr( $categories_class . ' ' . $add_class ); ?>">
	<?php if ( 'default' === $hover_style ) { ?>
		...
		<?php if ( true === $show_title ) { ?>
			<div class="portfolio-description">
				...
				<p><i class="fa fa-tag"></i><?php polo_render( $categories_names ); ?></p>
			</div>
		...
		<?php if ( true === $show_excerpt ) { ?>
			<div class="portfolio-details">
				...
				if ( isset( $post_meta['portfolio_description'] ) && ! empty( $post_meta['portfolio_description'] ) ) {
					...
					$portfolio_description = polo_do_multilang_text($portfolio_description);
	...
	<?php portfolio_hover_effects($hover_style,$thumb,$folio_excerpt, $width, $height);?>
